...
class TestGaff2:
    @pytest.mark.parametrize(
        "pdb, prm, value",
        [("tests/data/lig.pdb", "tests/data/lig-prm-single-lj.xml",
          22.77804946899414)])
    ...
    def test_gaff2_total(self, pdb, prm, values):
        ...
        for ii in range(pos.shape[0]):
            for jj in range(ii + 1, pos.shape[0]):
                pairs.append((ii, jj))
        pairs = np.array(pairs, dtype=int)
        totalEref = sum(values)
        totalEfunc = h.getPotentialFunc()
        params = h.getParameters()
        totalEcalc = totalEfunc(pos, box, pairs, params)
        npt.assert_almost_equal(totalEcalc, totalEref, decimal=3)