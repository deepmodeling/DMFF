...
#include "../src/SchAlg/octreeSchAlgGPU.cuh"
...
namespace dpnblist {
    ...
    void nblist_test_OctreeGPU(std::string filename, std::string reffilename, float r_cutoff, float skin = 0.0){
        ...
        OctreeSearchGPU s_alg(box, xyz, r_cutoff);
        s_alg.search();
    ...
    TEST_SUITE("Test Ref Octree List")
    {
        ...
        TEST_CASE("Octree List on GPU"){
        // test1
            nblist_test_OctreeGPU("heterogeneous.lmp", "sort_output_heterogeneous.txt", 3.0, 0.0);
