...
#include "../src/SchAlg/cellSchAlgCPU.h"
...
namespace dpnblist {
    ...
    void nblist_test_CellCPU(std::string filename, std::string reffilename, float r_cutoff, float skin = 0.0){
        ...
        CellSearchCPU s_alg(box, xyz, r_cutoff);
        s_alg.search();
    ...
    TEST_SUITE("Test Ref Cell List")
    {
        ...
        TEST_CASE("Cell List on CPU"){
        // test1
            nblist_test_CellCPU("heterogeneous.lmp", "sort_output_heterogeneous.txt", 3.0, 0.0);
