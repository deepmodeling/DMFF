    ...
    def __init__(
      ...
      #      const_vals = const_vals - np.sum(const_vals) / len(const_vals)
        ...
        if damp_mod == 1:
            ...
            raise ValueError("damp_mod must be 1, 2 ,3 or 4")
    ...
    def generate_get_energy(self):
       # @jit_condition()
        ...
        def get_energy(positions, box, pairs, mscales, eta, chi, J, aux=None):
            ...
            b_vector = jnp.concatenate((-chi, self.const_vals))
           
            A = E_hession(self.init_q, self.init_lagmt, chi, J, pos, box, pairs, eta, ds, buffer_scales, mscales)
           ...
           # M = jnp.zeres((len(pos)+n_const,len(pos)+n_const))
